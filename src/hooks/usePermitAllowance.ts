...
export function usePermitAllowance(token?: Token, owner?: string, spender?: string) {
  ...
  const contract = useContract<Permit2>(getPermit2(chainId), PERMIT2_ABI)
...
export function useUpdatePermitAllowance(
  ...
  return useCallback(async () => {
    try {
      ...
      const { domain, types, values } = AllowanceTransfer.getPermitData(permit, getPermit2(chainId), chainId)
