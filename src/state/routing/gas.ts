...
export async function getApproveInfo(
  ...
  try {
    const allowance = await tokenContract.callStatic.allowance(account, getPermit2(currency.chainId))
  ...
  try {
    const approveTx = await tokenContract.populateTransaction.approve(getPermit2(currency.chainId), MaxUint256)
